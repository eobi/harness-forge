...
| **OGHarn** | coverage vs the developer harness | +14% median (1.14x bar) | JSON libs 0.82–0.86x; **codec via AUTO-composition 1.04x** vs the simple dev harness (hand-tuned 1.29x; 0.73x of the maximally-tuned advanced one) | **MIXED, measured** — below on simple JSON parsers; **AUTO-composition now BEATS the simple developer harness on the codec archetype (libwebp, 1.04x, generated with no human) by folding decode subsystems and auto-fuzzing decode options; a maximally-tuned developer harness still leads. First measured result of the GENERATOR beating a developer harness. |
