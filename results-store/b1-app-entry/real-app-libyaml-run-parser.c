 ...
 *
 *   harness : run_parser_run_parser_main
 *   target  : run_parser
 *   producer: app_lift
...
/* APPLICATION harness: input arrives through the 'argv' channel, not an API call. */
#ifdef __cplusplus
extern "C"
#endif
...
#endif
    char *hf_argv[] = { "run_parser", hf_path, NULL };
    volatile long hf_sink = (long)run_parser_main((int)(sizeof(hf_argv)/sizeof(hf_argv[0])) - 1, hf_argv);
    (void)hf_sink;
#ifdef _WIN32
    DeleteFileA(hf_path);
#else
