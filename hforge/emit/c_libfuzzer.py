...
def _ns_arg(a: str, idx: int) -> str:
    """Namespace every harness local token (hf_aN / hf_outN / hf_lenN) inside an argument by
    fold index, so a fold does not collide; the (type)hf_data / (type)hf_size casts and scalar
    defaults carry no such token and are left untouched."""
    return _re.sub(r"\b(hf_(?:a|out|len)\d+)\b", rf"\1_{idx}", a)


def _ns_arg_legacy(a: str, idx: int) -> str:
    """Namespace an out-local argument (&hf_aN / hf_aN) by fold index; leave casts/scalars."""
    import re as _re
    m = _re.match(r"^(&?)(hf_a\d+)$", a.strip())
    return f"{m.group(1)}{m.group(2)}_{idx}" if m else a


def _emit_app_entry(ir: HarnessIR, *, with_driver: bool = True) -> "Emitted":
    """Emit a harness for an APPLICATION entry point rather than a library API sequence.

    The fuzzer's bytes reach the target through a CHANNEL, because an application has no API
    to call: a real main(argc, argv) is handed a path, a parse-file function is handed a
    path, a parse-buffer function is handed the bytes directly. For the two path channels the
    harness writes the input to a private temp file, drives the entry, and unlinks -- the
    exact shape a GUI file-drop and a mobile share-sheet specialise later.

    THE TEMP FILE IS PER-INVOCATION AND ALWAYS REMOVED. A fixed path would let one input's
    ...
    if ae.header and ae.header not in _headers(ir):
        incs += f'\n#include "{ae.header}"'
    sink = "volatile long hf_sink = " if ae.returns_int else ""
    ...
    if getattr(ae, "app_seq", None):
        ...
        def _buffer_block(e, idx: int) -> str:
            ...
            # several entries does not collide, and a pointer return is freed when a
            # deallocator is known, so a long campaign over a codec does not OOM.
            body = ""
            for l in (e.call_locals or []):
                # Namespace the declared identifier by fold index, handling `T v = {0};`,
                # `T *v = ...;` and array decls `static unsigned char v[N];` alike.
                head = l.split("=")[0].split("[")[0].replace(";", " ")
                toks = head.strip().split()
                nm = toks[-1].lstrip("*") if toks else ""
                body += "    " + (_re.sub(rf"\b{nm}\b", f"{nm}_{idx}", l) if nm else l) + "\n"
            ...
            if e.call_args:
                ...
                args = ", ".join(_ns_arg(a, idx) for a in e.call_args)
                callexpr = f"{e.symbol}({args})"
            else:
                callexpr = f"{e.symbol}((const char *)hf_data, hf_size)"
        ...
        needs_tmp = True
        if ae.channel == APP_ARGV:
            argv = ae.argv or [ir.target.name or "app", "@INPUT@"]
            elems = []
            for a in argv:
                elems.append("hf_path" if a == "@INPUT@" else f'"{a}"')
            argv_c = ", ".join(elems + ["NULL"])
...
def emit(ir: HarnessIR, *, with_driver: bool = True) -> Emitted:
    if getattr(ir, "app_entry", None) is not None:
        return _emit_app_entry(ir, with_driver=with_driver)
