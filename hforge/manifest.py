...
PHASES: tuple = (
    ...
    Phase("P3", "producers: test-lift, LLM->IR, graph traversal", PARTIAL, (
        ...
        Deliverable("P3.PARSER_CONSTRUCTORS",
                    ...
                    PLANNED,
                    note="SUPERSEDED — the diagnosis below was WRONG and the correction is "
                         "worth more than the original. json_loadb never reaches the "
                         "constructor-ranking path at all. It has no handle parameter, so it "
                         "belongs to _free_function_plans, the same path that carries "
                         "cmsOpenProfileFromMem — and there `_stream_bind` returns None for "
                         "it. Not for the buffer: slices are built for `buffer` and "
                         "`mode_flags` successfully. It gives up on `json_error_t *error`, a "
                         "pointer to a COMPLETE struct the callee fills with why the parse "
                         "failed. That is a caller-allocated out-parameter, exactly the shape "
                         "already modelled for `yaml_parser_scan(parser, &token)`: declare it "
                         "inline and pass its address. Refusing the whole plan instead costs "
                         "the entry point. I checked two earlier hypotheses against the code "
                         "and both were wrong — that `char` was poisoned as a handle by "
                         "`char *json_dumps`, and that ranking picked another constructor. "
                         "Neither survived contact: `known` holds only json_t. REAL FIX: in "
                         "the free-function path, a pointer to a complete struct is an "
                         "out-parameter, not a refusal. ORIGINAL AND INCORRECT DIAGNOSIS: "
        ...
        Deliverable("P3.EMPTY_PROFILE",
                    ...
                    note="jbig2dec published 0.00% on 25,353,303 executions in run-016, in a "
                         ...
                         "is. INCOMPLETE, and measured as such: the cap truncates AFTER the "
                         "campaign ends, so peak DISK use is still unbounded — leptonica "
                         "reached 656 MB in run-018 printing `Error in pixReadMemPnm` per "
                         "rejected input, and jbig2dec's equivalent was 2.5 GB. Redirecting "
                         "to a file removed the memory blowup, which was the cause of the "
                         "lost profile, and left the disk one. The cap has to apply DURING "
                         "the run — a size-limited writer rather than a post-hoc truncate. "
                         "Talkative libraries are the norm, not the exception: two of the "
                         "five new targets do this — and an empty or unmergeable profile is now reported as NOT "
