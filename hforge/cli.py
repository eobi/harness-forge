...
def cmd_emit(args) -> int:
    ...
    print(f"wrote {out/_h}")
    print(f"wrote {out/_d}   (replay: the campaign binary ignores stdin)")
...
def cmd_certify(args) -> int:
    ...
    # The device oracle is ADDITIONAL to the host gates, never a substitute: the host build
    # above is where discovery happens (instrumentation is cheap); the device run proves the
    # path reaches where the target actually runs. It is opt-in and its absence is NOT_RUN.
    if getattr(args, "run_on_device", False):
        print()
        if not _mobile_family(ir):
            print("  device gate: NOT RUN — plan targets no mobile platform")
        else:
            import tempfile
            work = Path(tempfile.mkdtemp(prefix="hforge-certdev-"))
            valid = _load_corpus(args.valid_corpus) or corpus.valid_only(ir).inputs
            res = _run_on_device_pipeline(ir, work, valid, verbose=args.verbose)
            _print_device_result(res)

    return {"certified": 0, "provisional": 0, "rejected": 1}[cert.verdict]


def _cc_preamble(ir) -> str:
    """The compiler variable the emitted build command actually references.

    The C++ backend's build command says `$CXX`. A preamble that set only `CC` produced a
    build.sh that aborted on `CXX: unbound variable` under `set -eu`, so EVERY C++ build
    was broken -- and no test caught it, because the C++ tests assert on the emitted
    source string and never run the script.
    """
    if (ir.target.language or "").lower() in ("c++", "cpp", "cxx", "c++17"):
        return 'CXX="${CXX:-clang++}"\n'
    return 'CC="${CC:-clang}"\n'
...
def _artifact_names(ir):
    ...
    # The same trap as the Java case above: clang compiles a `.c` file as C, so a C++
    # harness written to harness.c fails on the first `std::` it meets.
    if (ir.target.language or "").lower() in ("c++", "cpp", "cxx", "c++17"):
        return "harness.cc", "driver.cc"
    return "harness.c", "driver.c"


_CXX_EXT = {".hpp", ".hh", ".hxx", ".h++", ".hp"}
_CXX_HINT = re.compile(r"^\s*namespace\s+\w+\s*\{|^\s*template\s*<|\bpublic\s*:|"
                       r"\bstd::(?:string|vector|unique_ptr)\b", re.M)


def looks_like_cxx(path: str) -> bool:
    """Whether this header should go to the C++ producer.

    The extension decides when it is unambiguous. A `.h` that declares a namespace, a
    template or an access specifier is C++ regardless of what it is called -- and handing
    such a header to the C producer is exactly the failure this guesses around: it parses
    to nothing and reports "no function consumes attacker-controllable input", which
    blames the library for a routing mistake.
    """
    if Path(path).suffix in _CXX_EXT:
        return True
    try:
        return bool(_CXX_HINT.search(Path(path).read_text(errors="replace")))
    except OSError:
        return False
...
def cmd_propose(args) -> int:
    ...
    if args.classpath:
        ...
        if lang == "auto":
            lang = "c++" if looks_like_cxx(args.header) else "c"
        if lang == "c++":
            from .producers import cxx_header
            tgt.language = "c++"
            plans = cxx_header.propose(headers, tgt, platforms=plats,
                                       knobs=Knobs(max_len=args.max_len))
        else:
    ...
    if not plans:
        ...
        if not args.classpath and getattr(args, "lang", "auto") == "c":
            if looks_like_cxx(args.header):
                print("note: this header looks like C++ but was parsed as C because "
                      "--lang=c was given. Re-run with --lang=c++.", file=sys.stderr)
        return 1
...
def cmd_app_lift(args) -> int:
    ...
    if args.out:
        ...
        print(e.source)
    if getattr(args, "run_on_device", False):
        if not _mobile_family(plan):
            print("  device gate: NOT RUN — plan targets no mobile platform "
                  "(pass --platform android-arm64-emulator or ios-arm64-simulator)")
        else:
            import tempfile
            work = Path(tempfile.mkdtemp(prefix="hforge-aldev-"))
            res = _run_on_device_pipeline(plan, work, _load_corpus(args.input_dir),
                                          verbose=getattr(args, "verbose", False))
            _print_device_result(res)
    return 0


# ── the mobile message surface (P7 producer) ──────────────────────────────────

def _mobile_family(ir) -> str:
    """Which on-device pipeline a plan's platforms name: 'android', 'ios-sim', or ''."""
    plats = list(ir.platforms)
...
def _write_mobile_bundle(out_dir: Path, ir, em) -> None:
    """Write harness.c, driver.c and a cross-build build.sh for a mobile plan. The build.sh
    build command is already the NDK / iphonesimulator cross-compile (the platform-aware
    emitter produced it), so the artifact an operator receives is a real cross-build, not a
    host build mislabelled with a platform comment."""
    out_dir.mkdir(parents=True, exist_ok=True)
    hname, dname = _artifact_names(ir)
    (out_dir / hname).write_text(em.source)
    if em.driver:
        (out_dir / dname).write_text(em.driver)
    fam = _mobile_family(ir)
    if fam == "android":
        run_note = ("# Run on a device with:  adb push <bin> /data/local/tmp/ && adb shell <bin> <input>\n"
                    "# `hforge run-on-device` automates push + run + the baseline DIFFERENTIAL\n"
                    "# and downgrades HWASan->ASan when the system image is not a _hwasan build.\n")
    elif fam == "ios-sim":
        run_note = ("# Run on the booted Simulator with:  xcrun simctl spawn <udid> <bin> <input>\n"
                    "# This is a REACHABILITY/replay build (ASan); libFuzzer's iossim runtime is\n"
                    "# absent, so discovery happens on the macOS host. `hforge run-on-device`\n"
                    "# automates spawn + the baseline differential.\n")
    else:
        run_note = ""
    (out_dir / "build.sh").write_text(
        "#!/bin/sh\n"
        "# Generated by Harness Forge (message-surface producer). Run from this directory.\n"
        f"# platforms: {', '.join(ir.platforms)}\n"
        "set -eu\n\n"
        + run_note
        + f"\n# instrumented build\n{' '.join(em.build_command)}\n\n"
        f"# uninstrumented baseline (for the on-device differential)\n"
        f"{' '.join(em.driver_build_command)}\n")
    (out_dir / "build.sh").chmod(0o755)


def _run_on_device_pipeline(ir, work: Path, inputs: list, *, verbose: bool = False) -> dict:
    """Build the plan's harness for its mobile platform and run it against `inputs` through the
    device oracle: build_android + push_and_run + run_differential for Android, build_ios_sim +
    the simctl differential for the iOS Simulator.

    Returns a structured result. Honours NOT_RUN honesty: when the toolchain, a device, or a
    baseline is missing, the outcome is an explicit 'not_run' with a reason — never a silent
    pass. Trust ceiling is respected: an iOS *device* is reachability-only and is never built
    or driven here (only the Simulator is).
    """
    fam = _mobile_family(ir)
    if not fam:
        return {"outcome": "not_run",
                "reason": f"plan platforms {ir.platforms} name no Android or iOS-Simulator "
                          f"target this pipeline can build and run"}
    try:
        em = emit(ir)
    except EmitError as e:
        return {"outcome": "not_run", "reason": f"emit refused: {e}"}
    work.mkdir(parents=True, exist_ok=True)
    hname, dname = _artifact_names(ir)
    (work / hname).write_text(em.source)
    (work / dname).write_text(em.driver)
    srcs = [work / hname, work / dname] + [Path(s) for s in ir.target.sources]
    incs = list(ir.target.include_dirs)
    data = inputs[0] if inputs else b"MMS1\x08\x00\x08\x00\x0c\x00\x00\x00payload"

    if fam == "android":
        if not tc.find_ndk():
            return {"outcome": "not_run", "reason": "no NDK: an Android harness cannot be "
                    "built here (set ANDROID_NDK_HOME)"}
        live = [d for d in dev.android_devices() if d.kind in ("device", "emulator")]
        serial = live[0].serial if live else None
        abi, api = _android_abi_for(ir)
        inst = dev.build_android(srcs, work, include_dirs=incs, abi=abi, api=api,
                                 serial=serial)
        base = dev.build_android(srcs, work, include_dirs=incs, abi=abi, api=api,
                                 detector="none", suffix="-baseline")
        if not inst.ok:
            return {"outcome": "not_run", "reason": f"cross-build failed: {inst.reason}",
                    "log": inst.log if verbose else inst.log[-200:]}
        if not serial:
            avds = dev.emulators_available()
            return {"outcome": "not_run", "built": str(inst.binary), "detector": inst.detector,
                    "reason": (f"built for {abi}/api{api} ({inst.detector}) but no device is "
                               f"attached; {len(avds)} AVD(s) available to boot" if avds
                               else "built, but no device attached and no AVD defined")}
        if not base.ok:
            return {"outcome": "not_run", "reason": "no uninstrumented baseline: a lone "
                    "instrumented run cannot tell a target fault from an instrumentation "
                    "artifact"}
        diff = dev.run_differential(serial, inst.binary, base.binary, data)
        return {"outcome": diff.verdict, "detail": diff.detail, "serial": serial,
                "detector": inst.detector,
                "downgraded": inst.downgraded, "downgrade_reason": inst.downgrade_reason}

    # ios-sim
    if not tc.find_xcrun():
        return {"outcome": "not_run", "reason": "no xcrun: the iOS Simulator toolchain is "
                "unavailable (install Xcode)"}
    pid = next((p for p in ir.platforms if p.startswith("ios-") and p.endswith("-simulator")),
               "ios-arm64-simulator")
    sims = [d for d in dev.ios_simulators() if d.kind == "simulator"]
    udid = sims[0].serial if sims else None
    inst = dev.build_ios_sim(srcs, work, platform_id=pid, detector="asan", include_dirs=incs)
    base = dev.build_ios_sim(srcs, work, platform_id=pid, detector="none",
                             include_dirs=incs, suffix="-baseline")
    if not inst.ok:
        return {"outcome": "not_run", "reason": f"iOS-sim cross-build failed: {inst.reason}",
                "log": inst.log if verbose else inst.log[-200:]}
    if not udid:
        return {"outcome": "not_run", "built": str(inst.binary),
                "reason": "built for the iOS Simulator but no simulator is booted "
                          "(xcrun simctl boot <name>)"}
    if not base.ok:
        return {"outcome": "not_run", "reason": "no uninstrumented baseline for the differential"}
    diff = dev.run_ios_differential(udid, inst.binary, base.binary, data)
    return {"outcome": diff.verdict, "detail": diff.detail, "udid": udid, "detector": "asan"}


def _android_abi_for(ir):
    from .emit.android_ndk import target_abi                         # noqa: PLC0415
    return target_abi(ir)


def _print_device_result(res: dict) -> None:
    oc = res.get("outcome")
    if oc == "not_run":
        print(f"  device gate: NOT RUN — {res.get('reason', 'unavailable')}")
        if res.get("built"):
            print(f"    (a binary WAS produced: {res['built']})")
        return
    tag = {tc.OK: "ok", tc.FAULT: "FAULT", dev.ARTIFACT: "instrumentation-artifact"}.get(oc, oc)
    where = res.get("serial") or res.get("udid") or "device"
    dn = f", detector={res['detector']}" if res.get("detector") else ""
    if res.get("downgraded"):
        dn += f" (downgraded: {res.get('downgrade_reason', '')})"
    print(f"  device gate [{where}{dn}]: {tag} — {res.get('detail', '')[:100]}")


def cmd_message_surface(args) -> int:
    """Discover a messaging app's SMS/PDU or attachment/media surface and emit a mobile harness.

    The mobile analogue of app-lift (P7 producer). It recognises the two zero-interaction
    shapes — SMS/PDU parsing and attachment/media decoding — defaults the plan to the mobile
    platforms, and (with --run-on-device) cross-builds and runs the on-device differential.
    """
    from .producers import message_surface                          # noqa: PLC0415
    tgt = Target(name=args.name or Path(args.header[0]).stem,
                 sources=list(args.source or []),
                 include_dirs=list(args.include or []))
    plan, rec = message_surface.propose(
        list(args.header), tgt, includes=tuple(args.include or []),
        only=args.only, surface=args.surface, platforms=(args.platform or None))
    print("sms candidates:   " + (", ".join(rec.get("sms_candidates", [])) or "none"))
    print("media candidates: " + (", ".join(rec.get("media_candidates", [])) or "none"))
    if plan is None:
        print(f"refused: {rec.get('why_not', '')}")
        return 1
    print(f"surface: {rec['surface']}   platforms: {', '.join(plan.platforms)}")
    if rec["surface"] == "media":
        ch = rec.get("chosen", {})
        print(f"chosen: fold {ch.get('folded', 0) + 1} decoders onto one input"
              + (f", free via {ch['free_symbol']}" if ch.get("free_symbol") else ""))
    else:
        print(f"chosen: {rec['chosen'].get('symbol', args.only)} via the "
              f"{rec['chosen'].get('channel', 'buffer')} channel")
    ...
    try:
        ...
        print(f"emit refused: {e}", file=sys.stderr)
        return 2
    print(f"emit backend: {_mobile_family(plan) or 'host'}   "
          f"build: {' '.join(em.build_command[:1] + ['...'] + em.build_command[-2:])}")
    if args.out:
        outp = Path(args.out)
        _write_mobile_bundle(outp, plan, em)
        hname, _ = _artifact_names(plan)
        print(f"wrote {outp/hname}, {outp/'build.sh'}")
    else:
        print(em.source)

    if args.run_on_device:
        import tempfile
        work = Path(tempfile.mkdtemp(prefix="hforge-msgdev-"))
        res = _run_on_device_pipeline(plan, work, _load_corpus(args.input_dir),
...
def cmd_run_on_device(args) -> int:
    """Cross-build a plan for its mobile platform and run it through the on-device differential.

    Android over adb (build_android + push_and_run + differential); the iOS Simulator over
    simctl. A missing NDK/xcrun, a missing device, or a missing baseline yields an explicit
    NOT RUN with a reason — never a silent pass.
    """
    ir = _load(args.ir)
    import tempfile
    work = Path(args.work) if args.work else Path(tempfile.mkdtemp(prefix="hforge-rod-"))
    inputs = _load_corpus(args.input_dir)
    if args.input:
        inputs = [Path(args.input).read_bytes()] + inputs
    res = _run_on_device_pipeline(ir, work, inputs, verbose=args.verbose)
    print(f"plan: {ir.name}   platforms: {', '.join(ir.platforms)}")
    _print_device_result(res)
    # NOT_RUN is not a pass and not a failure of the target: exit 2 so a script can tell it
    # apart from a clean run (0) and a real fault (1).
    return {tc.OK: 0, tc.FAULT: 1, dev.ARTIFACT: 0, "not_run": 2}.get(res["outcome"], 2)
...
def main(argv=None) -> int:
    ...
    c.add_argument("--run-on-device", action="store_true", dest="run_on_device",
                   help="additionally cross-build and run the on-device differential when the "
                        "plan targets a mobile platform (Android/iOS Simulator)")
    ...
    al.add_argument("--run-on-device", action="store_true", dest="run_on_device",
                    help="after emitting, cross-build and run the on-device differential "
                         "for the plan's mobile platform (Android/iOS Simulator)")
    al.add_argument("--input-dir", default="",
                    help="corpus directory whose first input drives --run-on-device")
    al.add_argument("-v", "--verbose", action="store_true")
    al.set_defaults(fn=cmd_app_lift)

    ms = sub.add_parser("message-surface",
                        help="discover a messaging app's SMS/PDU or attachment/media surface "
                             "and emit a mobile harness (P7 producer)")
    ms.add_argument("--header", action="append", required=True,
                    help="the messaging library's public header (repeatable)")
    ms.add_argument("--include", action="append", default=[],
                    help="extra -I directory (repeatable)")
    ms.add_argument("--source", action="append", default=[],
                    help="library source compiled with the harness (repeatable)")
    ms.add_argument("--name", default="")
    ms.add_argument("--surface", choices=["auto", "sms", "media"], default="auto",
                    help="which surface to target: sms (PDU/short-message parsing), media "
                         "(attachment/codec fold, the zero-click shape), or auto")
    ms.add_argument("--only", default="",
                    help="lift this exact symbol as the (single-entry) message parser")
    ms.add_argument("--platform", action="append",
                    help="override the mobile target platform(s); default android-arm64-"
                         "emulator + ios-arm64-simulator")
    ms.add_argument("--out", default="",
                    help="write harness.c/driver.c/build.sh to this DIRECTORY instead of stdout")
    ms.add_argument("--force", action="store_true", help="emit even if a static gate blocks")
    ms.add_argument("--run-on-device", action="store_true", dest="run_on_device",
                    help="cross-build and run the on-device differential after emitting")
    ms.add_argument("--input-dir", default="",
                    help="corpus directory whose first input drives --run-on-device")
    ms.add_argument("-v", "--verbose", action="store_true")
    ms.set_defaults(fn=cmd_message_surface)

    rod = sub.add_parser("run-on-device",
                         help="cross-build a plan for its mobile platform and run the "
                              "on-device differential (Android over adb, iOS over simctl)")
    rod.add_argument("ir", help="the .hir.json plan to build and run")
    rod.add_argument("--input", help="a single crash/seed input file to run")
    rod.add_argument("--input-dir", default="", help="a corpus directory to run")
    rod.add_argument("--work", help="build directory (default: a temp dir)")
    rod.add_argument("-v", "--verbose", action="store_true")
    rod.set_defaults(fn=cmd_run_on_device)
    ...
    for _p in (pr, b):
        _p.add_argument("--lang", choices=["auto", "c", "c++"], default="auto",
                        help="which producer reads the header. 'auto' routes "
                             ".hpp/.hh, and any header declaring a namespace, "
                             "template or access specifier, to the C++ producer")
